...
function link(href: string, contents: string | JSX.Element): JSX.Element {
    return (
        <span><a href={href}>{contents}</a><span className={"print-only"}> ({href})</span></span>
    )
}

function colour(hex: string): JSX.Element {
    hex = "#" + hex
...
function colourRow(name: string, hex: string): JSX.Element {
    return (
        <tr>
            <td>{name}</td>
            <td>{colour(hex)}</td>
...
function colourTable(): JSX.Element {
    return (
        <span>
